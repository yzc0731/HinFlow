...
def sample_tracks_nearest_to_points(track, points):
        B, V, track_len, N, _ = track.shape
        track = track.reshape(B*V, track_len, N, 2)
        start_pos = track[:, 0] # (B*V, N, 2)
        start_pos = start_pos[:, :, None, :] # (B*V, N, 1, 2)

        # Process the points for different situations
        if points.dim() == 2:
            # (n, 2)
            ...
            if batch_size == 1 and B > 1:
                points = points.repeat(B, 1, 1) # (1, n, 2) -> (B, n, 2)
            # Oh, I think this logic is wrong! Because here B means the total video length. 
            points = points[:, None, :, :].repeat_interleave(V, dim=0) # (B*V, 1, num_points, 2)
        elif points.dim() == 4:
            # (1, v, n, 2) or (B, v, n, 2)
            batch_size, V, num_points, _ = points.shape
            if batch_size == 1 and B > 1:
                points = points.repeat(B, 1, 1, 1) # (1, v, n, 2) -> (B, v, n, 2)
            points = points.reshape(B*V, 1, num_points, 2) # B=199=video_len, V=2, num_points=30, 2
        dist = torch.norm(start_pos - points, dim=-1)  # (B*V, N, num_points)
        nearest_idx = torch.argmin(dist, dim=1) # (B*V, num_points)
        nearest_tracks = torch.stack([track[i, :, nearest_idx[i]] for i in range(B*V)], dim=0) # (B*V, 16, num_points, 2)
        nearest_tracks = nearest_tracks.reshape(B, V, track_len, -1, 2)
...
def tracks_to_binary_img(tracks, img_size):
    ...
    img = F.conv2d(img, kernel, padding=padding)[:, 0, :, :]
    ...
    assert torch.max(img) <= 1, torch.max(img).item()
    return img
...
def combine_track_and_img(track: torch.Tensor, vid: np.ndarray):
    ...
    """
    if vid.shape[-1] == 3:
        # vid shape is B, H, W, C
        vid = np.transpose(vid, (0, 3, 1, 2)) # B, C, H, W
