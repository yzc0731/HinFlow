...
class ExtraModalityTokens(nn.Module):
    def __init__(
        ...
        if robot == "Panda":
            ...
            ee_dim = 3
            tcp_pose_dim = 7 # only for maniskill

        self.num_extra = int(use_joint) + int(use_gripper) + int(use_ee) + int(use_tcp)
        # by default in panda is 2

        extra_low_level_feature_dim = (
            int(use_joint) * joint_states_dim
            + int(use_gripper) * gripper_states_dim
            + int(use_ee) * ee_dim
            + int(use_tcp) * tcp_pose_dim
        )
        ...
        for (proprio_dim, use_modality, modality_name) in [
            ...
            (tcp_pose_dim, self.use_tcp, "tcp_pose"),
        ]:
    ...
    def forward(self, obs_dict):
        ...
        for (use_modality, modality_name) in [
            (self.use_joint, "joint_states"), # by default is True
            (self.use_gripper, "gripper_states"), # by default is True
            (self.use_ee, "ee_states"), # by default is False
            (self.use_tcp, "tcp_pose"),
        ]:
